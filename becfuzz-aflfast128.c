...
EXP_ST u8  virgin_bits[MAP_SIZE128],     /* Regions yet untouched by fuzzing */
           ...
           virgin_new_in_old[MAP_SIZE128];  /* Bits we haven't seen in new paths saved by old edges */
...
//becfuzz
struct collide_path{
    u16 coll_index;                      /* index in path_table */
    u32 coll_cksum;                     /* checksum of collided path */
...
/* instrument target binary and get the size of bitmap*/
// static void instrument_binary(){
//     // instrument
//     char * dyninst_args_tracer[] = {"BECFuzzDyninst", "-i", target_path, "-o", inst_binary, "-b", becfuzz_dir, NULL};
//     execute(dyninst_args_tracer, "BECFuzzDyninst", 1);

//     //get the value of max total edges and max conditional edges
//     u8* tmp;
//     FILE *fp;

//     tmp = alloc_printf("%s/%s", becfuzz_dir, NUM_EDGE_FILE);
//     fp = fopen(tmp, "rb");
//     if(!fp){
//         PFATAL("open address file '%s' failed!\n", tmp);
//     }
//     int ret = fscanf(fp,"%d %d", &Dyn_Conditional_Size, &Dyn_Map_Size);
//     if(ret == 2) {
//         OKF("Read bitmap size successfully!");
//         ACTF("conditional size:%d, map size:%d", Dyn_Conditional_Size, Dyn_Map_Size); //rosen
//     }
//     else{
//         PFATAL("Read bitmap size failed!\n");
//     }
//     fclose(fp);
//     ck_free(tmp);
...
/* Is it a new path?
hash table and hash chain to identify paths;
    is new path? or which old path? 
    return: 0: old path with collision
            1: old path without collision
            2: new path with collidion
...
EXP_ST void setup_shm(void) {
  ...
  edge_trace = ck_alloc(MAP_SIZE128 >> 3); //for path trace

  if (!in_bitmap) memset(virgin_bits, 255, MAP_SIZE128);

  memset(virgin_tmout, 255, MAP_SIZE128);
  memset(virgin_crash, 255, MAP_SIZE128);
  memset(virgin_new_in_old, 255, MAP_SIZE128);
 
  //shm_id = shmget(IPC_PRIVATE, MAP_SIZE128, IPC_CREAT | IPC_EXCL | 0600);
  shm_id = shmget(IPC_PRIVATE, MAP_SIZE128 + 1, IPC_CREAT | IPC_EXCL | 0600); //rosen-- the last one is for indirect edges
...
static u8 run_target(char** argv, u32 timeout) {
  ...
  if (dumb_mode == 1 || no_forkserver) {
    ...
    if (trace_bits[MAP_SIZE128] == 1){ //rosen- new indirect edges
...
static u8 save_if_interesting(char** argv, void* mem, u32 len, u8 fault) {
  ...
  memset(edge_trace, 0, MAP_SIZE128 >> 3);
  minimize_bits(edge_trace, trace_bits);
  path_cksum = hash32(edge_trace, MAP_SIZE128 >> 3, HASH_CONST); //rosen
...
int main(int argc, char** argv) {
  ...
  SAYF(cCYA "becfuzz-afl " cBRI VERSION cRST " by nsclab\n");
  ...
  while ((opt = getopt(argc, argv, "+i:o:f:m:t:T:dnCB:S:M:x:p:")) > 0)
