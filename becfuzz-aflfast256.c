...
EXP_ST u8  virgin_bits[MAP_SIZE256],     /* Regions yet untouched by fuzzing */
           ...
           virgin_new_in_old[MAP_SIZE256];  /* Bits we haven't seen in new paths saved by old edges */
...
EXP_ST void setup_shm(void) {
  ...
  edge_trace = ck_alloc(MAP_SIZE256 >> 3); //for path trace

  if (!in_bitmap) memset(virgin_bits, 255, MAP_SIZE256);

  memset(virgin_tmout, 255, MAP_SIZE256);
  memset(virgin_crash, 255, MAP_SIZE256);
  memset(virgin_new_in_old, 255, MAP_SIZE256);
 
  //shm_id = shmget(IPC_PRIVATE, MAP_SIZE256, IPC_CREAT | IPC_EXCL | 0600);
  shm_id = shmget(IPC_PRIVATE, MAP_SIZE256 + 1, IPC_CREAT | IPC_EXCL | 0600); //rosen-- the last one is for indirect edges
...
static u8 run_target(char** argv, u32 timeout) {
  ...
  if (dumb_mode == 1 || no_forkserver) {
    ...
    if (trace_bits[MAP_SIZE256] == 1){ //rosen- new indirect edges
